...
class TestObjectStore(unittest.TestCase):
    ...
    def test_get_pano_csvs(self):
        panorama_csvs = self.objs._get_csv_type('2016', '03/21/TMX7315120208-000021/', 'panorama')
    ...
    def test_get_trajectory_csvs(self):
        trajectory_csvs = self.objs._get_csv_type('2016', '03/17/TMX7315120208-000020/', 'trajectory')
    ...
    def test_get_csvs(self):
        panorama_csvs = self.objs.get_csvs('panorama')
    ...
    def test_get_panorama_store_object(self):
        panorama_csv = self.objs._get_csv_type('2016', '03/21/TMX7315120208-000021/', 'panorama')[0]
        csv = self.objs.get_panorama_store_object(panorama_csv)
        self.assertIsNotNone(csv)

    def test_get_csv_to_read_runs(self):
        panorama_csv = self.objs._get_csv_type('2016', '03/21/TMX7315120208-000021/', 'panorama')[0]
        response = self.objs.get_panorama_store_object(panorama_csv)
        import csv
        rows = csv.reader(iter(response.decode("utf-8").split('\n')), delimiter='\t', quotechar=None, quoting=csv.QUOTE_NONE)
        headers = next(rows)
        print(headers)
        for row in rows:
            print(row)
